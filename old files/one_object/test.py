...
class Environment:
    ...
    def __init__(self):
        self.body_list = []
        self.world = world(gravity=(0, -10), sleep=False)
        self.colour_list = []
    ...
    def reset(self):
        ...
        self.stick = self.world.CreateDynamicBody(
            position=(pos1, pos2),
            shapes=polygonShape(box=(60, 6)),
            angle=0,
        )
        self.stick.CreatePolygonFixture(box=(60,6), density=0.1, friction=0)
        offset = 2.5
        DENSITY_2 = 0.1
        FRICTION = 10
        if self.obj_type == 5:
...
for frame in range(start, max_frames):
    ...
    if done:
        ...
        if len(episode_rewards) % 1 == 0:
            print('Train Episode Reward: '+ str(np.mean(episode_rewards[-100:])))
