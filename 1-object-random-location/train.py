...
class Environment:
    ...
    def __init__(self):
        self.body_list = []
        self.world = world(gravity=(0, -5000), sleep=False)
        self.colour_list = []
...
for frame in range(start, max_frames):
    ...
    agent_grab.replay_buffer.push(state1, action, reward, terminal)
    episode_reward += reward


    if len(agent_grab.replay_buffer) >= (50000):
        agent_grab.epsilon = exp_eps_sched.get_eps(frame)

        if num_act == 3:
            eval_bool = True


            agent_grab.update(batch_size)
    ...
    if frame % 10000 == 0:
        agent_grab.model2.load_state_dict(agent_grab.model1.state_dict())

    #if frame % 5000 == 0 and frame != 0 and eval_bool == True:
           #tb.save_value('Train Loss', 'train_loss', frame, loss1.item())
           #tb.save_value('Epsilon', 'epsilon', frame, agent_grab.epsilon)

    if done:
        curr_agent = 'grab'
        episode_rewards.append(episode_reward)
        if len(episode_rewards) % 1 == 0:
            print('Train Episode Reward: '+ str(round(np.mean(episode_rewards[-100:]),2)))
            print('Current Episode Reward: ' + str(round(episode_reward,2)))
        ...
        if len(episode_rewards) % 50 == 0 and eval_bool:
          ...
          for i in range(10):
            ...
            while not done:
              ...
              action = agent_grab.get_action(state)
              curr_reward += reward
            eval_r_list.append(curr_reward)
            curr_reward = 0
          eval_rewards.append(np.mean(eval_r_list))
          print('=========================================================')
          print('Current Eval Reward: ' + str(round(np.mean(eval_r_list),2)))
          print('=========================================================')
